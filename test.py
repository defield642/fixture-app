...
# Initialize state with proper error handling and recovery
    ...
    try:
        if os.path.exists(SAVE_FILE):
            with open(SAVE_FILE, "r", encoding="utf-8") as f:
                ...
                print(f"Groups in state: {list(loaded_state.get('groups', {}).keys())}")
                STATE = loaded_state
                return True
        return False
    except json.JSONDecodeError as e:
        print(f"JSON decode error in {SAVE_FILE}: {e}")
        # Try to create backup of corrupted file
        try:
            if os.path.exists(SAVE_FILE):
                backup_file = f"{SAVE_FILE}.corrupted_backup"
                import shutil
        ...
        print(f"Error loading state from {SAVE_FILE}: {e}")
        return False

# Main state initialization
if os.path.exists(SAVE_FILE):
    if not load_state():
        print("Failed to load state, using default")
        STATE = create_default_state()
        # Try to save the fresh state
        try:
            with open(SAVE_FILE, "w", encoding="utf-8") as f:
                json.dump(STATE, f, ensure_ascii=False, indent=2)
            print("Created new state file with default structure")
        except Exception as e:
            print(f"Warning: Could not save default state: {e}")
else:
...
def ensure_state_structure():
    """Ensure all required state structures exist with proper types"""
    required_structures = {
        "users": {},
        "leagues": {},
        "global_chats": {},
    ...
    # Ensure all main structures exist
    for key, default_value in required_structures.items():
        if key not in STATE:
            print(f"Initializing missing structure: {key}")
            STATE[key] = default_value

    # Ensure numeric IDs are proper integers
    for key, default_value in numeric_ids.items():
        if key not in STATE or not isinstance(STATE[key], int):
            print(f"Initializing/resetting numeric ID: {key}")
            STATE[key] = default_value
    ...
    for username, user_data in STATE.get("users", {}).items():
        ...
        for field, default_value in required_user_fields.items():
            if field not in user_data:
                user_data[field] = default_value

    print("State structure verified and repaired if necessary")

# FIX: Enhanced save_state function
def save_state():
    """Save state to JSON file with proper error handling"""
    try:
        ...
        print(f"Groups in saved state: {list(STATE.get('groups', {}).keys())}")
        return True

    except Exception as e:
        print(f"Error saving state to {SAVE_FILE}: {e}")
    ...
    try:
        ...
        try:
            with open(SAVE_FILE, "r", encoding="utf-8") as f:
                ...
                print("Recovered corrupted JSON file")
                return True
        except:
            print("Could not recover JSON file, using default state")
            STATE = create_default_state()
            save_state()
            return False
    except Exception as e:
...
def sanitize_message_content(message):
    """Sanitize message content to prevent JSON and security issues"""
    if not message:
        return ""
...
# Updated message saving functions
def save_private_message(sender, receiver, message, message_type="text", content=None, edited=False, reply_to=None):
    ...
    # Ensure private_chats structure exists
    if "private_chats" not in STATE:
        STATE["private_chats"] = {}
    ...
    sanitized_message = sanitize_message_content(message)
    ...
    message_data = {
        ...
        "type": message_type,
        "content": content,  # For media, this is base64 encoded which is safe
        "timestamp": datetime.datetime.now().isoformat(),
        "read": False,
        "edited": edited,
        "reply_to": reply_to,
    ...
    # Limit chat history to prevent JSON file from growing too large
    if len(STATE["private_chats"][chat_id]) > 500:  # Reduced from 1000
        STATE["private_chats"][chat_id] = STATE["private_chats"][chat_id][-500:]

    # Save state with error handling
    if save_state():
        return message_data
    else:
        print("Failed to save message to state")
        return None
...
def debug_state():
    ...
    state_info = {
        ...
        "file_size": os.path.getsize(SAVE_FILE) if os.path.exists(SAVE_FILE) else 0,
        "users_count": len(STATE.get("users", {})),
        "private_chats_count": len(STATE.get("private_chats", {})),
        "groups_count": len(STATE.get("groups", {})),
        "last_save": "Unknown"
    }
...
def register():
    if request.method == "POST":
        ...
        if ' ' in username:
            return render_template_string(REGISTER_HTML, error="Username cannot contain spaces")
...
def admin_panel():
    ...
    # Create a safe users dictionary that handles usernames with spaces
    safe_users = {}
    for username in list(pending_users) + list(verified_users):
        if username in STATE["users"]:
            safe_users[username] = STATE["users"][username]
...
def admin_verify_user(username):
    ...
    # URL decode the username in case it contains spaces or special characters
    import urllib.parse
...
def handle_private_message(data):
    ...
    # Sanitize and validate reply_to
    if reply_to:
        try:
            # Ensure reply_to is a simple dict, not a complex object
            if isinstance(reply_to, dict):
                reply_to = {
                    'id': int(reply_to.get('id', 0)),
                    'message': sanitize_message_content(reply_to.get('message', '')),
...
<head>
    ...
    <style>
        ...
        /* Hide sidebar on mobile, show only chat list */
        @media (max-width: 768px) {
            .desktop-only {
...
<body class="min-h-screen mobile-full-height mobile-no-padding mobile-no-margin">
    <!-- Mobile Header -->
    <header class="bg-white mobile-header mobile-rounded-none shadow-sm flex items-center justify-between px-4 py-3">
        <div class="flex items-center">
            <button onclick="goBack()" class="p-2 mr-3 text-purple-700 desktop-only">
                <i class="fas fa-arrow-left"></i>
            </button>
            <div>
                <h1 class="text-xl font-bold text-purple-800">Chats</h1>
                <p class="text-xs text-purple-600">{{ user_chats|length }} conversations</p>
            </div>
        </div>
        <div class="flex items-center space-x-3">
            <button id="darkModeToggle" class="p-2 text-purple-700">
                <i id="darkModeIcon" class="fas fa-moon"></i>
            </button>
            <button onclick="toggleSearch()" class="p-2 text-purple-700">
    ...
    <div id="searchBar" class="bg-white hidden px-4 py-3 border-b border-gray-200">
        <div class="relative">
            <input type="text" placeholder="Search conversations..."
                   class="w-full p-3 pl-10 bg-gray-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500">
            <i class="fas fa-search absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400"></i>
        </div>
    </div>

    <!-- Mobile Chat List (WhatsApp Style) -->
    <div class="mobile-chat-area bg-white mobile-chat-list mobile-rounded-none">
        <!-- Your Chats List -->
        <div class="space-y-0">
            {% for chat in user_chats %}
            <a href="/chat/{{ chat.user }}" class="block mobile-chat-item touch-target transition-all hover:bg-gray-50 active:bg-gray-100">
                <div class="flex items-center space-x-3">
                    <!-- User Avatar -->
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center justify-between mb-1">
                            <h3 class="font-semibold text-gray-800 truncate text-base">{{ chat.user }}</h3>
                            {% if chat.last_message %}
                            <span class="text-xs text-gray-500 flex-shrink-0 ml-2">
                                {{ chat.last_message.timestamp[11:16] }}
                            </span>
                            {% endif %}
                        </div>
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            {% if chat.unread > 0 %}
                            <div class="unread-badge flex-shrink-0 ml-2">
                                {{ chat.unread }}
                            </div>
                            {% endif %}
                        </div>
        ...
        <div class="border-t border-gray-200 mt-4 pt-4">
            <h3 class="text-sm font-semibold text-gray-800 px-4 mb-3">Start New Chat</h3>
            <div class="space-y-0">
                {% for user in users %}
                <a href="/chat/{{ user }}" class="block mobile-chat-item touch-target transition-all hover:bg-gray-50 active:bg-gray-100">
                    <div class="flex items-center space-x-3">
                        <div class="relative flex-shrink-0">
        ...
        <!-- Empty State -->
        {% if not user_chats and not users %}
        <div class="flex-1 flex flex-col items-center justify-center p-8 text-center">
            ...
            <h3 class="text-lg font-semibold text-gray-800 mb-2">No Chats Yet</h3>
            <p class="text-gray-600 text-sm mb-4">Start a conversation with other players</p>
        </div>
        {% endif %}
    </div>

    <!-- Desktop Layout (Hidden on Mobile) -->
    <div class="desktop-only max-w-6xl mx-auto px-4 py-6">
        <header class="bg-white rounded-2xl shadow-md p-6 mb-6 flex justify-between items-center">
            ...
            <div class="flex gap-3">
                <button id="darkModeToggleDesktop" class="px-4 py-2 rounded-xl bg-purple-100 text-purple-700 hover:bg-purple-200 transition">
                    <i id="darkModeIconDesktop" class="fas fa-moon mr-2"></i>Theme
                </button>
    ...
    <div class="bottom-nav desktop-only:hidden">
        <div class="flex justify-around items-center">
            <a href="/status" class="flex flex-col items-center text-purple-600">
            ...
            <a href="/chat" class="flex flex-col items-center text-purple-600">
                <i class="fas fa-comments nav-icon"></i>
                <span class="nav-text">Chats</span>
            </a>
            <a href="/groups" class="flex flex-col items-center text-gray-500">
    ...
    <script>
        ...
        function initializeDarkMode() {
            const darkModeToggles = document.querySelectorAll('#darkModeToggle, #darkModeToggleDesktop');
            const darkModeIcons = document.querySelectorAll('#darkModeIcon, #darkModeIconDesktop');
                ...
                <table class="min-w-full text-sm">
                    ...
                    <tbody>
                        ...
                        <tr class="border-b border-gray-200">
                            ...
                            <td class="p-3">
                                {% if users[username] and users[username].team_name %}
                            ...
                            <td class="p-3">
                                {% if users[username] and users[username].joined_at %}
                                    {{ users[username].joined_at[:10] }}
                                {% else %}
                                    N/A
                                {% endif %}
                            </td>
...
<body class="min-h-screen">
    ...
    <div class="max-w-7xl mx-auto px-2 sm:px-4 py-2">
        ...
        <!-- Chat Section - Clean Redirect to Dedicated Chat Page -->
        <div id="chatSection" class="content-section">
            <div class="bg-white rounded-2xl shadow-md p-8 text-center">
                <div class="max-w-md mx-auto">
                    ...
                    <h2 class="text-2xl font-bold text-purple-800 mb-4">Welcome to Chat</h2>
                    <p class="text-gray-600 mb-6">Connect with other players through private messaging with our dedicated chat interface</p>
                    <div class="space-y-4">
                        <a href="/chat" class="w-full bg-purple-600 text-white py-3 px-6 rounded-lg hover:bg-purple-700 transition flex items-center justify-center">
                        ...
                        <div class="text-sm text-gray-500 bg-gray-50 rounded-lg p-4">
                            <div class="flex justify-between mb-2">
                                ...
                                <span class="font-semibold">{{ user_chats|length }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span>Users available:</span>
                                <span class="font-semibold">{{ users|length - 1 }}</span>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        document.getElementById('globalSearch').addEventListener('input', function(e) {
            ...
            // Search in leagues
            const leagueItems = document.querySelectorAll('#leaguesSection .border');
            ...
            // Search in groups
            const groupItems = document.querySelectorAll('#groupsSection .group-item');
            groupItems.forEach(item => {
                const text = item.textContent.toLowerCase();
